...
namespace Graphics {
	...
	OGL *OGL::_instance = nullptr;

	OGL::OGL()
	{
		assert(!_instance && "You cannot init GL twice!");
	...
	OGL *OGL::getInstance(){
		return OGL::_instance;
	}

	int OGL::InitGL(GLvoid){
	...
	GLvoid OGL::ReSizeGLScene(GLsizei width, GLsizei height)		// Resize And Initialize The GL Window
	{
	...
	GLvoid OGL::KillGLWindow(GLvoid)
	{
	...
	BOOL OGL::CreateGLWindow(char* title, int width, int height, int bits, bool fullscreenflag){
								 ...
								 LPARAM	lParam)	// Additional Message Information
	{
		...
		case WM_ACTIVATE:							// Watch For Window Activate Message
		{
			...
			{
				gl->active = TRUE;						// Program Is Active
			}
			else
			{
				gl->active = FALSE;						// Program Is No Longer Active
			}
		...
		{
			gl->keys[wParam] = TRUE;					// If So, Mark It As TRUE
			return 0;								// Jump Back
		}

		case WM_KEYUP:								// Has A Key Been Released?
		{
			gl->keys[wParam] = FALSE;					// If So, Mark It As FALSE
			return 0;								// Jump Back
		}

		case WM_SIZE:								// Resize The OpenGL Window
		{
			gl->ReSizeGLScene(LOWORD(lParam), HIWORD(lParam));  // LoWord=Width, HiWord=Height
			return 0;								// Jump Back
		}
		}
